...
public class CameraMovement : MonoBehaviour
{
    ...
    public float keyboardMovementSpeedFast = 10f; //speed with keyboard movement
    public float screenEdgeMovementSpeed = 3f; //spee with screen edge movement
    public float followingSpeed = 5f; //speed when following a target
    public float rotationSped = 3f;
    public float panningSpeed = 10f;
    public float mouseRotationSpeedHorizontal = 10f;
    public float mouseRotationSpeedVertical = 20f;
    ...
    private void Start()
    {
        ...
        childTransform = gameObject.GetComponentInChildren<Camera>().gameObject.transform;
    }
    ...
    private void Move()
    {
        if (useKeyboardInput)
        {
            ...
            {
                desiredMove *= keyboardMovementSpeedFast;
            }
            else
            {
    ...
    private void Rotation()
    {
        ...
        {
            m_Transform.Rotate(Vector3.up, MouseAxis.x * Time.deltaTime * mouseRotationSpeedHorizontal, Space.World);
            m_Transform.Rotate(Vector3.right, -MouseAxis.y * Time.deltaTime * mouseRotationSpeedVertical, Space.Self);
        }
    }

    private void Zoom()
    {
        //Vector3 moveVector = transform.up * Input.GetAxisRaw("Mouse ScrollWheel") * zoomSpeed * Time.deltaTime;
        childTransform.Translate(Vector3.forward *Input.GetAxisRaw("Mouse ScrollWheel") * zoomSpeed * Time.deltaTime, Space.Self);
    }
